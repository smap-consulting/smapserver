	...
	if(!showCompleted) {
		for(i= 0, j = 0; i < featuresObj.length; i++) {
			if(featuresObj[i].attributes.assignment_status !== "submitted" && 
					...
					(abs(featuresObj[i].geometry.x) > 0.01 || abs(featuresObj[i].geometry.y) > 0.01)) {
		...
		for(i= 0, j = 0; i < featuresObj.length; i++) {
			if(featuresObj[i].geometry && (abs(featuresObj[i].geometry.x) > 0.01 || abs(featuresObj[i].geometry.y) > 0.01)) {
				featuresToLoad.push(featuresObj[i]);
				j++;
			}
		}
